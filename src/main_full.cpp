...
void demoDoublyLinkedList() {
    printSeparator("DOUBLY LINKED LIST DEMO");
    
    DoublyLinkedList<int> dlist;
    
    std::cout << "Добавляем элементы: 1, 2, 3, 4" << std::endl;
    dlist.push_back(1);
    dlist.push_back(2);
    dlist.push_front(3);
    dlist.push_back(4);
    
    std::cout << "Список вперед: ";
    dlist.printForward();
    
    std::cout << "Список назад: ";
    dlist.printBackward();
    
    std::cout << "Размер: " << dlist.getSize() << std::endl;
    
    std::cout << "\nВставляем 99 на позицию 2:" << std::endl;
    dlist.insert(2, 99);
    dlist.printForward();
    ...
    dlist.serialize("doubly_list.txt");
    std::cout << "\nСохранено в doubly_list.txt" << std::endl;
}
...
int main() {
    ...
    std::cout << "  - doubly_list.txt" << std::endl;
